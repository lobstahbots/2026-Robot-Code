...
fun trenchAssist(input: ChassisSpeeds, robotPose: Pose2d): ChassisSpeeds {
    ...
    val xComponent: Double =
        if ((FieldConstants.LinesVertical.allianceZone.meters - DriverAssistConstants.TRENCH_ASSIST_RADIUS * normalized.vxMetersPerSecond <= robotPose.measureX) && robotPose.x <= FieldConstants.LinesVertical.neutralZoneNear && normalized.vxMetersPerSecond > 0) (DriverAssistConstants.TRENCH_ASSIST_STRENGTH * (FieldConstants.LinesVertical.neutralZoneNear - robotPose.x))
        else if (FieldConstants.LinesVertical.allianceZone <= robotPose.x && (robotPose.measureX <= FieldConstants.LinesVertical.neutralZoneNear.meters - DriverAssistConstants.TRENCH_ASSIST_RADIUS * normalized.vxMetersPerSecond) && normalized.vxMetersPerSecond < 0) (DriverAssistConstants.TRENCH_ASSIST_STRENGTH * (FieldConstants.LinesVertical.allianceZone - robotPose.x))
        else if ((FieldConstants.LinesVertical.neutralZoneFar.meters - DriverAssistConstants.TRENCH_ASSIST_RADIUS * normalized.vxMetersPerSecond <= robotPose.measureX) && robotPose.x <= FieldConstants.LinesVertical.oppAllianceZone && normalized.vxMetersPerSecond > 0) (DriverAssistConstants.TRENCH_ASSIST_STRENGTH * (FieldConstants.LinesVertical.oppAllianceZone - robotPose.x))
        else if (FieldConstants.LinesVertical.neutralZoneFar <= robotPose.x && (robotPose.measureX <= FieldConstants.LinesVertical.oppAllianceZone.meters - DriverAssistConstants.TRENCH_ASSIST_RADIUS * normalized.vxMetersPerSecond) && normalized.vxMetersPerSecond < 0) (DriverAssistConstants.TRENCH_ASSIST_STRENGTH * (FieldConstants.LinesVertical.neutralZoneFar - robotPose.x))
        else return ChassisSpeeds()
